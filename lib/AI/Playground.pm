...
use AI::Model::Config qw(get_api_info get_all_models get_api_key);
...
sub new {
    ...
    my $self = bless {
        port => $args{port} || $ENV{PORT} || $< || 4001,
        model => $args{model} || $ENV{API_MOD} || 'claude',
        secure_mode => $args{secure_mode} || $ENV{SECURE_MODE} || 0,
        instance_id => $args{instance_id} || $ENV{INSTANCE_ID} || $ENV{USER} || "ai-$<",
        script_log_dir => $args{script_log_dir} || "ai_scripts",
        log_dir => $args{log_dir} || "ai_logs",
        debug => defined($args{debug}) ? $args{debug} : ($ENV{DEBUG} // 1),
        socket_path => $args{socket_path} || "ai-socket",
    }, $class;
...
sub start {
    ...
    my $api_info = get_api_info($self->{model});
    $self->{api_info} = $api_info;
    $self->{instance_type} = $api_info->{provider};
...
=head2 env_show_var(@vars)

Shows environment variables, hiding API keys.
...
sub env_show {
    ...
    return $self->env_show_var(qw(API_KEY API_MOD));
}
...
sub _init_environment {
    ...
    die "Environment incomplete. Required: API_KEY, API_MOD" unless $self->env_show();
    
    # Get API info based on model name
    my $api_info = get_api_info($self->{model});
    say STDERR "Using API URL: " . $api_info->{url}->{api};
...
sub execute_script {
    ...
    # Log the script
    my $log_dir = path("$self->{script_log_dir}/$ai_user");
    $log_dir->mkpath unless $log_dir->exists;
    my $timestamp = time();
